...
if(isset($_POST['engTitle'], $_POST['chTitle'])){
    ...
    if(isset($_FILES["image-upload"]) && $_FILES["image-upload"]["error"] == 0){
        ...
        if(!array_key_exists($ext, $allowed)){
            ...
            echo 'location.href = "../knowledge.php";</script>';
        }
    ...
    if($_POST['blogId'] != null && $_POST['blogId'] != ''){
        if($uploadOk == 1){
            if ($update_stmt = $db->prepare("UPDATE knowledge SET title_en=?, title_ch=?, en=?, ch=?, img=? WHERE id=?")) {
            ...
            if ($update_stmt = $db->prepare("UPDATE knowledge SET title_en=?, title_ch=?, en=?, ch=? WHERE id=?")) {
        ...
        if($uploadOk == 1){
            if ($insert_stmt = $db->prepare("INSERT INTO knowledge (title_en, title_ch, en, ch, img) VALUES (?, ?, ?, ?, ?)")) {
            ...
            if ($insert_stmt = $db->prepare("INSERT INTO knowledge (title_en, title_ch, en, ch) VALUES (?, ?, ?, ?)")) {
